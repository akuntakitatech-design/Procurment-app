...
COMPOSE_FILE="$ROOT_DIR/deploy/stock-opname-integrity-test.compose.yml"
ENV_FILE="${HOME}/.procurement-saas-staging.env"
PROJECT_NAME="procurement-stock-opname-integrity-test"
LOG_FILE="/tmp/procurement-stock-opname-integrity-test.log"
...
echo "==> Menyiapkan Stock Opname integrity test terpisah"
echo "==> Database test: procurement_stock_opname_integrity_test"
...
docker compose -p "$PROJECT_NAME" --env-file "$ENV_FILE" -f "$COMPOSE_FILE" up \
  ...
  --exit-code-from stock-opname-runner \
  2>&1 | tee "$LOG_FILE"
...
if [ "$TEST_EXIT" -ne 0 ]; then
  echo
  echo "==> FAIL: Stock Opname integrity test gagal (exit $TEST_EXIT)"
...
echo
echo "==> PASS: Stock Opname integrity test selesai"
