...
COMPOSE_FILE="$ROOT_DIR/deploy/loan-return-integrity-test.compose.yml"
ENV_FILE="${HOME}/.procurement-saas-staging.env"
PROJECT_NAME="procurement-loan-return-integrity-test"
LOG_FILE="/tmp/procurement-loan-return-integrity-test.log"
...
echo "==> Menyiapkan Loan & Return integrity test terpisah"
echo "==> Database test: procurement_loan_return_integrity_test"
...
docker compose -p "$PROJECT_NAME" --env-file "$ENV_FILE" -f "$COMPOSE_FILE" up \
  ...
  --exit-code-from loan-return-runner \
  2>&1 | tee "$LOG_FILE"
...
if [ "$TEST_EXIT" -ne 0 ]; then
  echo
  echo "==> FAIL: Loan & Return integrity test gagal (exit $TEST_EXIT)"
...
echo
 echo "==> PASS: Loan & Return integrity test selesai"
